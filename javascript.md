...
Yukarıdaki bu işlemimizi birde arrow function ile yapalım.
...
Yukarıdaki fonksiyon yazımlarını incelediğimizde yazdığımız kod uzunluğu kısalmış ve arrow function kullanarak daha kısa kod yazarak aynı sonuca ulaşmış olduk.

**- Callback Functions / async-await**
Öncelikle gecikme olayını kavramamız gerekir. Bu nedenle aşağıdaki kodları inceleyip uygulayalım.
...
}, 2000);  // her iki saniyede bir çalışır (durdurmadığımız sürece)
```

Aşağıdaki örnekte ise callback fonksiyonunun kullanımı gösterilmiştir.
...
İşlemden de anlaşılacağıüzere _sayilariTopla_ isimli fonksiyonumuza _sumNumbers_ isimli bir callback foksiyonu gönderdik ve işlemimizi yaptırdık.

**fetch foksiyonu**'nu inceleyelim:
...
Yukarıdaki işlemde ise önce userların nameleri ekrana yazdırılır ve daha sonra ise postlar ekrana yazdırılır. Dikkat etmemiz gereken olay sırasıdır. Şimdi ise aşağıdaki kodlarıinceleyelim.
...
Üstteki kodları her çalıştırdığımızda gelen data sıralaması değişecektir. Çünkü kullandığımız bu fonksiyonlar asenkron işlemler olduğundan gelen datalarda sıralama değişebilir. Yani ilk önce posts'lar gelebilir. Bazen ise users dataları gelebilir. Bu durumda **işlemleri sıraya koymamız gerekir.**
...
Yukarıdaki gibi bir fonksiyon kullanmadan _anonim bir foksiyon_ kullanarak da yapabiliriz.
...
yukarıdaki fonksiyonu async olarak da tanımlayabiliriz.
...
**fetch işlemini anonim fonksiyon kullanarak yapalım**
...
**- Promises**
_Promise_ ile de aynı asenkron işlemler yapılmaktadır. _Promise_ kullanarak biz yapıya _"tamam, ben sana söz veriyorum bir promise geri döndürücen döndüremezsem de bir hata döndürücem"_ demiş oluyoruz. Buna göre datalarımız geliyor.
...
getCommnets(0)
  ...
  .catch((e) => console.log(e));  //  Bir problem oluştu
```
